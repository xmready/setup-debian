...
1. Download both `SHA256SUMS_DESKTOP` & `SHA256SUMS_DESKTOP.sign` from the [latest release](https://github.com/xmready/setup-debian/releases) to `/tmp`
2. Import [xmready's PGP signing key](https://keys.openpgp.org/vks/v1/by-fingerprint/31310B484B30ADABE8527D0E17AF13F5D2F5013A)
```
gpg --keyserver "hkps://keys.openpgp.org" --recv-keys 17AF13F5D2F5013A
```
3. Verify the PGP signature
```
gpg --verify "/tmp/SHA256SUMS_DESKTOP.sign" "/tmp/SHA256SUMS_DESKTOP"
```
4. Download `setup-desktop.sh` to `tmp`
```
curl \
   --fail \
   --location \
   --output-dir "/tmp" \
   --remote-name \
   "https://raw.githubusercontent.com/xmready/setup-debian/main/setup-desktop.sh"
```
5. Verify the checksum of `setup-desktop.sh` against `SHA256SUMS_DESKTOP`
```
cd "/tmp" && sha256sum --ignore-missing -c "SHA256SUMS_DESKTOP"
```
...
1. Download both `SHA256SUMS_SERVER` & `SHA256SUMS_SERVER.sign` from the [latest release](https://github.com/xmready/setup-debian/releases) to `/tmp`
2. Import [xmready's PGP signing key](https://keys.openpgp.org/vks/v1/by-fingerprint/31310B484B30ADABE8527D0E17AF13F5D2F5013A)
```
gpg --keyserver "hkps://keys.openpgp.org" --recv-keys 17AF13F5D2F5013A
```
3. Verify the PGP signature
```
gpg --verify "/tmp/SHA256SUMS_SERVER.sign" "/tmp/SHA256SUMS_SERVER"
```
4. Download `setup-server.sh` to `tmp`
```
curl \
   --fail \
   --location \
   --output-dir "/tmp" \
   --remote-name \
   "https://raw.githubusercontent.com/xmready/setup-debian/main/setup-server.sh"
```
5. Verify the checksum of `setup-server.sh` against `SHA256SUMS_SERVER`
```
cd "/tmp" && sha256sum --ignore-missing -c "SHA256SUMS_SERVER"
```
6. Run the setup script
```
