...
tmux_config_url="${remote_raw_url}/${remote_name}/main/configs/.tmux.conf"
...
&& printf '\n%s\n' \
  ...
  'stty -ixon' \
  'eval "$(fzf --bash)"' \
  '[[ -z $TMUX && -z $VIMRUNTIME ]] && { tmux attach || tmux new -s ${USER}; }' \
  >> "$bashrc_path" \
&& success 'bashrc customized' \
&& sleep 3 \
&& info 'downloading tmux config file...' \
&& curl --fail --location --output "$tmux_config_path" "$tmux_config_url" \
