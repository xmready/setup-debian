...
script_files=(
  ...
  "setup-desktop-homebrew.sh"
  "setup-desktop-clamav.sh"
  "setup-desktop-nvm.sh"
  "setup-desktop-ghcup.sh"
  "setup-desktop-kmonad.sh"
...
cleanup() {
  local status=$?
  if [[ "$status" -eq 0 ]]; then
    rm -f -- /tmp/setup-*.sh /tmp/SHA256SUMS_*
  else
    rm -f -- /tmp/setup-desktop-*.sh /tmp/setup-apt*.sh
  fi
}
...
if sudo shutdown --reboot +10; then
  info "SYSTEM WILL REBOOT IN 10 MINUTES"
  exit 0
else
