...
if [ -d "./scripts" ]; then
  cd "./scripts" || { err "Error: changing to scripts directory failed"; exit 1; }
fi
...
if [ ! "$(basename "$PWD")" = "scripts" ]; then
  err "Error: must be in the setup-debian/scripts/ directory"
  exit 1
fi
...
if ! sha256sum setup-desktop* setup-apt-clean.sh setup-nvm.sh > "$sha256sums_desktop_path"; then
  err "Error: desktop checksum generation failed"
  exit 1
fi

if ! sha256sum setup-server* setup-apt-clean.sh > "$sha256sums_server_path"; then
  err "Error: server checksum generation failed"
  exit 1
fi
